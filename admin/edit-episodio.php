        ...
        <div class="flex flex-col flex-1">
            ...
            <main class="h-full pb-16 overflow-y-auto">
                <div class="container px-6 mx-auto grid">
                    ...
                    <?php
                    $id = $_GET['id'];
                    $sql = mysqli_query($conn, "SELECT * FROM episodio WHERE idEpisodio = '$id'");
                    $row = mysqli_fetch_assoc($sql);
                    ?>
                    <div class="episodio px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                        <form action="">
                            <div class="errortxt"></div>
                            <label class="block text-sm">
                                <span class="text-gray-700 dark:text-gray-400">Name</span>
                                <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-red-400 focus:outline-none focus:shadow-outline-red dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Piloto" name="nome" value="<?php echo $row['nome']; ?>" />
                            </label>

                            <label class="block mt-4 text-sm">
                                <span class="text-gray-700 dark:text-gray-400">
                                    Anime - <?php
                                            $FK_idAnime = $row['FK_idAnime'];
                                            $sqlanime = mysqli_query($conn, "SELECT * FROM anime WHERE idAnime = '$FK_idAnime'");
                                            $rowanime = mysqli_fetch_assoc($sqlanime);
                                            echo $rowanime['nome']; ?>
                                </span>
                                <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-red-400 focus:outline-none focus:shadow-outline-red dark:focus:shadow-outline-gray" name="id_categoria" id="id_categoria">
                            ...
                            <span class="text-gray-700 dark:text-gray-400">
                                    Temporada - <?php
                                            $FK_idTemporada = $row['FK_idTemporada'];
                                            $sqltemporada = mysqli_query($conn, "SELECT * FROM temporada WHERE idTemporada = '$FK_idTemporada'");
                                            $rowtemporada = mysqli_fetch_assoc($sqltemporada);
                                            echo $rowtemporada['nome']; ?>
                                </span>
                                <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-red-400 focus:outline-none focus:shadow-outline-red dark:focus:shadow-outline-gray" name="id_sub_categoria" id="id_sub_categoria">
                                ...
                                <div class="input__item">
                                    ...
                                    <div class="file-upload" style="max-width: 300px">
                                        <div class="file-select">
                                            ...
                                            ?>
                                            <div class="file-select-button" id="fileName"><?php echo $msg; ?></div>
                            ...
                            <div>
                                <br>
                                <button class="px-10 py-4 font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-lg active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red">
                                    Cadastrar
                                </button>
                            </div>
